/**
 * Wondwerk Portrait Generator — Server Route
 * Roept de Claude API aan om een persoonlijk portret te genereren
 * op basis van de Wondwerk-testscores.
 *
 * Bestand: server/routes/wondwerk-portrait.ts
 */
...
const STRUCTURE_INFO: Record<string, { name: string; fullName: string; description: string; wound: string; gift: string; challenge: string }> = {
  A: {
    name: "De Ontwijker",
    fullName: "Ontwijkende structuur",
  ...
  E: {
    name: "De Presteerder",
    fullName: "Presterende structuur",
...
function buildSystemPrompt(): string {
  return `Je bent een ervaren, warm en psychologisch geschoold schrijver die werkt in de traditie van de lichaamsgerichte psychotherapie (Reich, Lowen, Pierrakos). Je schrijft persoonlijke portretten voor deelnemers die Wondwerk hebben ingevuld — een test die zes karakterstructuren meet.
...
BELANGRIJK:
- Gebruik NOOIT de klinische termen (schizoïd, oraal, symbiotisch, psychopathisch, masochistisch, rigide) in de lopende tekst. Alleen de nieuwe namen (ontwijkend, zoekend, versmeltend, strategisch, dragend, presterend).
...
function buildUserPrompt(data: {
  ...
  let prompt = `Schrijf een persoonlijk Wondwerk-portret voor iemand met het volgende profiel:
